...
def sarah_m(w_init, full_grad_func, stoch_grad_func, n, b, m, eta, beta, max_epochs,
    ...
    return_grad_history : If True, also return a list of ||G_k|| norms per iteration
    """

    w = np.copy(w_init)
    grad_history = [] if return_grad_history else None
    ...
    for s in range(max_epochs):
        ...
        if return_grad_history:
            grad_history.append(float(np.linalg.norm(G_0)))
        ...
        for k in range(1, m + 1):
            ...
            if return_grad_history:
                grad_history.append(float(np.linalg.norm(G_k)))
