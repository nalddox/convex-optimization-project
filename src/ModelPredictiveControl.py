...
class ModelPredictiveControl(object):
    ...
    def __init__(self,A,B,C,f,v,W3,W4,x0,desiredControlTrajectoryTotal,K):
    ...
    def propagateDynamics(self,controlInput,state):
        ...
        # add random noise (uncertainty for stochastic model)
        noise_std = 0.04
        w_k = np.random.normal(loc=0.0, scale=noise_std, size=(self.n, 1))
    ...
    def computeControlInputs(self):
        ...
        # compute the control sequence
        inputSequenceComputed=np.matmul(self.gainMatrix,vectorS)
        inputNominal=inputSequenceComputed[0:self.m, :]   # shape (m,1)
        inputFeedback=np.matmul(self.K, (actual_state-nominal_state))
        inputApplied=inputNominal + inputFeedback
